 ...
 * This may only be used after the omap_pmic_controller_ops setup function is
 * called.
 *
 ...
 *			constraints.
 * @setup:		Setup the device and associated registers.
 *			- Once this succeeds, this returns the pointer to
 *			the controller device and all other operations are
 *			expected to be ready for functionality. PMIC ops can
 *			be used in this function.
 * @voltage_set_vsel:	set the vsel - expected to be synchronous.
 * @voltage_get_vsel:	get the current vsel in micro-volts set on PMIC.
 *
...
struct omap_pmic_controller_ops {
	...
	int (*voltage_get_range) (struct device *control_dev, u32 *min_uv,
				  u32 *max_uv);
	int (*setup) (struct device *control_dev);
	int (*voltage_set_vsel) (struct device *control_dev, u8 vsel);
	int (*voltage_get_vsel) (struct device *control_dev, u8 *vsel);
};
