 ...
 * @vp:	pointer to voltage processor
 * @vsel: voltage to setup the voltage processor with
 */
static int omap_vp_set_init_voltage(struct omap_vp *vp, u8 vsel)
{
	struct regmap *regmap = vp->regmap;
	const struct omap_vp_reg_data *regs = vp->regs;
	int ret;
...
/**
 * omap_vp_get_current_vsel() - get the current voltage processor voltage
 * @vp:	pointer to voltage processor
 * @vsel: returns with voltage in vsel if read was successful.
 */
static int omap_vp_get_current_vsel(struct omap_vp *vp, u8 *vsel)
{
	struct device *dev = vp->dev;
	struct regmap *regmap = vp->regmap;
	const struct omap_vp_reg_data *regs = vp->regs;
 ...
 * @vp:			pointer to voltage processor
 * @target_vsel:	voltage to set the PMIC in vsel
 *
 * This will wait for the slew duration to ensure that the voltage is sync-ed
 * on the PMIC.
 */
static int omap_vp_forceupdate_scale(struct omap_vp *vp, u8 target_vsel)
{
	struct device *dev = vp->dev;
	struct regmap *regmap = vp->regmap;
	const struct omap_vp_reg_data *regs = vp->regs;
	...
	ret = omap_vp_get_current_vsel(vp, &old_vsel);
	if (ret) {
		dev_warn_ratelimited(dev,
	...
	if (timeout >= max_timeout) {
		dev_warn_ratelimited(dev,
				     ...
				     __func__, max_timeout, target_vsel);
		return -ETIMEDOUT;
	}

	ret = omap_vp_set_init_voltage(vp, target_vsel);
	if (ret) {
		dev_warn_ratelimited(dev,
				     "%s: Fail set init voltage at v=%d(%d)\n",
				     __func__, target_vsel, ret);
		return ret;
	}
	 ...
	 * the required level.
	 */
	smps_transition_uv = abs(target_vsel - old_vsel) * pinfo->step_size_uV;
	...
	ret = omap_vc_channel_set_on_voltage(vp->vc, target_vsel);
	if (ret) {
		dev_warn_ratelimited(dev,
 ...
 * @dev:	VP device pointer to voltage processor
 */
static int omap_vp_setup(struct device *dev)
{
	...
	if (!vp) {
		dev_err(dev, "%s: Invalid vp data\n", __func__);
		return -EINVAL;
	}

	if (!vp->pmic || !vp->vc || !vp->regmap || !vp->regs) {
		dev_err(dev, "%s:Invalid internal data structure\n", __func__);
		return -EINVAL;
	}
	regmap = vp->regmap;
	regs = vp->regs;
	...
	pmic = vp->pmic;
	if (!pmic->info || !pmic->ops) {
		dev_err(dev, "%s:Invalid PMIC data structure\n", __func__);
		return -EINVAL;
	}
	pinfo = pmic->info;
	ops = pmic->ops;

	/* Setup one VP at a time */
	mutex_lock(&omap_vp_list_mutex);
	ret = omap_vc_channel_setup(vp->vc);
	if (ret)
		goto out;
	...
	if (ret) {
		dev_err(dev, "%s: Convert min(%d) fail %d\n", __func__,
			vp->min_uV, ret);
		goto out;
	}
	ret = ops->uv_to_vsel(pmic, vp->max_uV, &vddmax);
	if (ret) {
		dev_err(dev, "%s: Convert max(%d) fail %d\n", __func__,
			vp->min_uV, ret);
		goto out;
	}
	...
	if (ret) {
		dev_err(dev, "%s: regmap write step_min fail %d\n",
			__func__, ret);
		goto out;
	}
	...
	if (ret) {
		dev_err(dev, "%s: regmap write step_max fail %d\n",
			__func__, ret);
		goto out;
	}
	...
	if (ret) {
		dev_err(dev, "%s: regmap write limitto fail %d\n",
			__func__, ret);
		goto out;
	}
	...
	if (ret) {
		dev_err(dev, "%s: regmap update config timeout fail %d\n",
			__func__, ret);
	}
...
/**
 * omap_vp_voltage_set_vsel() - controller operation to set voltage
 * @dev:	VP device to set voltage
 * @vsel:	voltage in vsel to set
 */
static int omap_vp_voltage_set_vsel(struct device *dev, u8 vsel)
{
	struct omap_vp *vp = dev_get_drvdata(dev);
	...
	return omap_vp_forceupdate_scale(vp, vsel);
}

/**
 * omap_vp_voltage_get_vsel() - controller operation to get voltage
 * @dev:	VP device to get voltage from
 * @vsel:	returns voltage in vsel if successful
 */
static int omap_vp_voltage_get_vsel(struct device *dev, u8 *vsel)
{
	struct omap_vp *vp = dev_get_drvdata(dev);
	...
	return omap_vp_get_current_vsel(vp, vsel);
}
...
static struct omap_pmic_controller_ops voltage_processor_ops = {
	...
	.setup = omap_vp_setup,
	.voltage_set_vsel = omap_vp_voltage_set_vsel,
	.voltage_get_vsel = omap_vp_voltage_get_vsel,
};
