 ...
 * error value.
 */
int omap_vc_channel_set_on_voltage(struct omap_vc_channel_info *info, u8 vsel)
{
	struct omap_vc *vc;
...
/**
 * omap_vc_channel_setup() - Setup VC channel
 * @info:	pointer to Voltage Controller channel info.
 */
int omap_vc_channel_setup(struct omap_vc_channel_info *info)
{
	struct omap_vc *vc;
	struct omap_vc_channel *vc_channel;
	struct device *dev;
	...
	vc_channel = info->ch;
	if (!vc_channel) {
		pr_err("Unable to find VC Channel data\n");
		return -EINVAL;
	}
	dev = vc_channel->dev;
...
out:
	mutex_unlock(&omap_vc_channel_list_mutex);
	if (ret)
		dev_err(dev, "Failed Setup VC channel with (%d)\n", ret);

	return ret;
}
EXPORT_SYMBOL_GPL(omap_vc_channel_setup);
...
out:
	if (ret) {
		...
		info = ERR_PTR(ret);
		dev_err(dev, "Failed request VC channel with (%d)\n", ret);
	}
